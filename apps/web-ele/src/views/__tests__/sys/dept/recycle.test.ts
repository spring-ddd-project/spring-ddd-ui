...
vi.mock('#/api/sys/dept', () => ({
  getDeptRecyclePage: vi.fn().mockResolvedValue({
    code: 0,
    data: { list: [], total: 0 },
  }),
  restoreDeptById: vi.fn().mockResolvedValue({ code: 0 }),
  wipeDeptById: vi.fn().mockResolvedValue({ code: 0 }),
}));
...
// Test component that mimics DeptRecycle
const TestDeptRecycle = {
  name: 'TestDeptRecycle',
  props: {
    gridApi: Object,
  },
  setup(props: any) {
  ...
  template: '<div class="test-dept-recycle"><slot /></div>',
};

describe('sys/dept/recycle', () => {
  it('should mount successfully', () => {
    const wrapper = mount(TestDeptRecycle, {
      props: {
